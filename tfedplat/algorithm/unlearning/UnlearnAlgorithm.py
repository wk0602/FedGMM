...
class UnlearnAlgorithm(fp.Algorithm):
    def __init__(
        ...
        **kwargs,
        ...
        self.max_unlearn_round = (
            params["UR"] if params is not None else int(0.5 * self.max_comm_round)
        )
        self.r_lr = params.get("r_lr", -1) if params is not None else -1
        ...
        self.early_stop_enabled = (
            params.get("early_stop", True) if params is not None else True
        )
        self.early_stop_threshold = (
            params.get("early_stop_threshold", 0.05) if params is not None else 0.05
        )  # ASR threshold
        self.early_stop_patience = (
            params.get("early_stop_patience", 10) if params is not None else 10
        )  # rounds without improvement
        self.early_stop_min_delta = (
            params.get("early_stop_min_delta", 0.001) if params is not None else 0.001
        )  # minimum ASR change
        self.best_asr = float("inf")
    ...
    class UnLearningCELoss:
        ...
        def __call__(self, pred, target):
            ...
            if self.reduction == "none":
                ...
                loss = -torch.mean(
                    torch.sum(torch.log(1.0 - pred / 2) * target_enc, dim=1)
                )
    ...
    def check_early_stopping(self):
        """Check if early stopping conditions are met based on ASR."""
        if not self.early_stop_enabled:
            return False

        # Calculate current ASR (Attack Success Rate)
        ...
        for i, metric_history in enumerate(self.metric_log["client_metric_history"]):
            if self.client_list[i].unlearn_flag:
                if len(metric_history["backdoor_test_accuracy"]) > 0:
                    test_acc = metric_history["backdoor_test_accuracy"][-1]
                    unlearned_client_local_backdoor_acc_list.append(
                        test_acc / 100.0
                    )  # Convert to [0, 1]
        ...
        current_asr = np.mean(unlearned_client_local_backdoor_acc_list)

        # Check if ASR is below threshold
        if current_asr <= self.early_stop_threshold:
            print(
                f"Early stopping: ASR ({current_asr:.4f}) <= threshold ({self.early_stop_threshold:.4f})"
            )
            return True

        # Check if ASR improved significantly
        if current_asr < self.best_asr - self.early_stop_min_delta:
            self.best_asr = current_asr
            self.patience_counter = 0
        else:
        ...
        if self.patience_counter >= self.early_stop_patience:
            print(
                f"Early stopping: No improvement for {self.early_stop_patience} rounds. Best ASR: {self.best_asr:.4f}, Current ASR: {current_asr:.4f}"
            )
            return True

        return False

    def save_unlearn_model(self):
        """Save the unlearned model and logs."""
        if self.save_model:
    ...
    def terminated(self):
        ...
        if self.check_early_stopping():
            self.save_unlearn_model()
            return True

        # Check if max unlearning rounds reached
        if self.current_comm_round >= self.max_unlearn_round:
            print(f"Reached max unlearning rounds: {self.max_unlearn_round}")
            self.save_unlearn_model()
            return True

        print(
            f"Unlearning stage - Round {self.current_comm_round}/{self.max_unlearn_round}"
        )
    ...
    def stat_update_conflict(self, d, gr_locals):
        ...
        if conflict_client_num > 0:
            out_log += (
                f"mean conflict coefficient: {conflict_angle / conflict_client_num} \n"
            )

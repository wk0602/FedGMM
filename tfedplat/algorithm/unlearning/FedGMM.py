...
class FedGMM(fp.UnlearnAlgorithm):
    ...
    - Stage 1 (unlearning): identify sensitive parameters via gradient magnitude,
    ...
    - Stage 2 (recovery): fall back to standard FedAvg-style aggregation on
      retained clients.
    """
    ...
    def __init__(self,
        ...
        super().__init__(name, data_loader, module, device, train_setting, client_num, client_list, online_client_num,
                         save_model, max_comm_round, max_training_num, epochs, save_name, outFunc, write_log,
                         dishonest, test_conflicts, params)
        self.mask_dict = {}  # client_id -> mask vector
        self.rho = float(params.get('gmm_rho', 0.05)) if params is not None else 0.05
        self.mask_refresh = int(params.get('gmm_mask_refresh', 0)) if params is not None else 0
        self.kl_weight = float(params.get('gmm_kl_weight', 1.0)) if params is not None else 1.0
        self.temperature = float(params.get('gmm_temperature', 1.0)) if params is not None else 1.0
        self.model_params = self.module.span_model_params_to_vec()

    class UniformKLLoss:
        def __init__(self, reduction='mean', temperature=1.0, weight=1.0, ignore_index=-100):
        ...
        def __call__(self, pred, target):
            ...
            loss_vec = -torch.sum(uniform * log_probs, dim=1)  # CE to uniform == KL(uniform || pred)
    ...
    def _compute_client_gradient(self, client):
        ...
        """
        msg = {'command': 'cal_gradient_loss', 'epochs': self.epochs, 'lr': self.lr, 'target_module': self.module}
    ...
    def _build_or_refresh_masks(self):
        ...
        for client in self.client_list:
            ...
            values, indices = torch.topk(torch.abs(grad_vec), k=k, largest=True, sorted=False)
    ...
    def _aggregate_masked_gradients(self, g_locals):
        ...
        weights = torch.Tensor(self.get_clinet_attr('local_training_number')).float().to(self.device)
    ...
    def _fedavg_recovery_step(self, m_locals):
        """Standard FedAvg aggregation for recovery stage."""
    ...
    def train_a_round(self):
        ...
        if not self.recovery_stage:
            ...
            # Recovery: normal FedAvg on retained clients.
            self._fedavg_recovery_step(m_locals)
            batch_num = torch.mean(torch.Tensor(self.get_clinet_attr('training_batch_num'))).item()
    ...
    def run(self):
        ...
        for client in self.client_list:
            if getattr(client, 'unlearn_flag', False):
                client.criterion = self.UniformKLLoss(temperature=self.temperature, weight=self.kl_weight)
